...
class ViF:
    ...
    def process(self, frames):
        ...
        for i in range(0, len(frames) - self.videoSubSampling - 5, self.videoSubSampling * 2):

            index += 1

            prev_f = frames[i + self.videoSubSampling]
            curr_f = frames[i + self.videoSubSampling * 2]
            next_f = frames[i + self.videoSubSampling * 3]
